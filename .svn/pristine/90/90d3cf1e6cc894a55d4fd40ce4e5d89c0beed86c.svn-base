<template>
    <div class="box">
        ...
        <el-row>
            <el-table ref="multipleTable" :data="tableData.slice((currentPage - 1) * pageSize, currentPage*pageSize)" height="calc(100vh - 230px)" border style="width: 100%" tooltip-effect="dark" @selection-change="handleSelectionChange" :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}" id="dzTable">
                ...
                <el-table-column prop="MILE" :label="'管段长度 | (KM)'" :render-header="renderheader" align="center"></el-table-column>
                <el-table-column prop="DSFNum" :label="'第三方|施工点数'" :render-header="renderheader" align="center"></el-table-column>
                <el-table-column prop="GFXNum" :label="'高风险|段数'" :render-header="renderheader" align="center"></el-table-column>
                <el-table-column prop="GHGNum" :label="'高后果|区数'" :render-header="renderheader" align="center"></el-table-column>
        ...
        operation(title){
            if(title=="修改段长信息"||title=="查看段长信息"){
                if(this.multipleSelection.length==0||this.multipleSelection.length>1){
                    ...
                    this.$myHttp.get('pipeMana/getPipeManInfo?sId='+myId).then((res) => {
           ...
           if(this.multipleSelection.length==0||this.multipleSelection.length>1){
                ...
                window.vm.$confirm('确定是否删除', {
                    ...
                    type: 'warning'
                       ...
                       this.$myHttp.get('pipeMana/deletePipeMana?userId='+this.$Cookie.get('userId') +'&id=' +id).then((res) => {
                               if (res.success) {
                                    window.vm.$message({
                                        type: 'success',
                                        message: res.msg,
       ...
       async submitDetail(e){
        ...
        getAll(e){
            ...
            return new Promise(resolve=>{
                 this.$myHttp.post('pipeMana/savePipeMana',e).then(async(res) => {
                        if (res.success) {
                            window.vm.$message({
                                type: 'success',
                                message: '操作成功',
        ...
        submitSearch(e){
            this.params=e;
            return this.$myHttp.post('pipeMana/getPipeManaList', e).then((res) => {
                      if (res.success) {
                          this.showSearch = false;
                          let arr=res.result;
        ...
        exportExcel(){
            ...
            //         document.querySelector("#dzTable")
            //     ); 
            ...
            //     }
            //     this.pageSize=tempSize; //表格还原
            //     return wbout;
            // });
             var that = this;
            // require.ensure([], () => {
            //     const { export_json_to_excel } = require('@/excel/export2Excel'); // 这里必须使用绝对路径，使用@/+存放export2Excel的路径
            //     const tHeader = ['输油处', '输油站', '管段名称','管段类型','起点桩号','终点桩号','管段长度','第三方施工点数','高风险段数','高后果区数','段长姓名','联系方式','备注']; // 导出的表头名信息
            //     const filterVal = ['AREANAME', 'STATIONNAME', 'ROUTENAME', 'TYPENAME', 'STARTSTAKE', 'ENDSTAKE', 'MILE', 'DSFNum', 'GFXNum', 'GHGNum','PERSONNAME', 'PHONE','NOTE',]; // 导出的表头字段名，需要导出表格字段名
            //     const list = that.tableData;
            //     const data = that.formatJson(filterVal, list);

            //     export_json_to_excel(tHeader, data, '段长信息');// 导出的表格名称，根据需要自己命名
            // });
            const tHeader = ['输油处', '输油站', '管段名称','管段类型','起点桩号','终点桩号','管段长度','第三方施工点数','高风险段数','高后果区数','段长姓名','联系方式','备注']; // 导出的表头名信息
            const filterVal = ['AREANAME', 'STATIONNAME', 'ROUTENAME', 'TYPENAME', 'STARTSTAKE', 'ENDSTAKE', 'MILE', 'DSFNum', 'GFXNum', 'GHGNum','PERSONNAME', 'PHONE','NOTE',]; // 导出的表头字段名，需要导出表格字段名
            const list = that.tableData;
