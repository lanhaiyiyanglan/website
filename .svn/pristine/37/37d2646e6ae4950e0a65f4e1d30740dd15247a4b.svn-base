...
export default {
  data() {
    ...
    return {
      ...
      loginRules: {
        username: [
          { required: true, trigger: "input", validator: validateUsername }
        ],
        password: [
          { required: true, trigger: "input", validator: validatePassword }
        ]
      },
      redirect: undefined
    };
  ...
  mounted(){
    ...
    if(this.$Cookie.get("loginName")!=undefined){
        if(this.loginForm.username==null||this.loginForm.username==undefined||this.loginForm.username==this.$Cookie.get("loginName")){
          this.loginForm.username=this.$Cookie.get("loginName")
  ...
  watch: {
    ...
    'loginForm.username':function(val){
       if(val==this.$Cookie.get("loginName")){
          this.loginForm.password=this.$Cookie.get("password");
          this.loginForm.remember=true;
       }else{
               ...
               this.$myHttp.get('gpsGroups/userLogin?userName=' +this.loginForm.username +'&password=' +this.loginForm.password).then((res) => {
                 ...
                 if(res.result.indexOf("sessionid")>-1){
                      ...
                      if(this.loginForm.remember){
                        this.$Cookie.set("loginName",this.loginForm.username,{ expires:365});
                        this.$Cookie.set("password",this.loginForm.password,{ expires:365});
                        this.$Cookie.set("remember",'true',{ expires:365});
                      }else{
                        this.$Cookie.set("loginName",'',{ expires: -1});
                        this.$Cookie.set("password",'',{ expires: -1});
                        this.$Cookie.set("remember",'false',{ expires: -1})
                      }
                 }else{
                   window.vm.$message({
                      type: 'error',
