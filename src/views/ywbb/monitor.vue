  ...
  <div class="box">
      ...
      <div class="container">
          <div class="wrapper" v-for="(item,index) in datas" :key="index"> 
              <p>{{item.diskName}}</p>
              <div class="chartsDom" :id="'chartDom'+index" :ref="'chartDom'+index">{{index}}</div>
              <ul class="lines">
       ...
       <div class="container2">
         <div id="charts2" ref="charts2"></div>
         <ul class="lines">
       ...
       <div class="container3">
         <div id="charts3" ref="charts3"></div>
         <ul class="lines">
                  <li><i></i>总计<span>100%</span></li>
                  <li><i></i>已用<span>{{cpuData.used}}%</span></li>
                  <li><i></i>未用<span>{{(100-cpuData.used).toFixed(2)}}%</span></li>
         </ul>
       </div>
  </div>
</template>
  ...
  methods: {
    ...
    drawPie(){
      for(var i = 0;i < this.datas.length;i++ ){
          var name='chartDom'+i;
          var myChart = this.$echarts.init(this.$refs[name][0]);
    ...
    drawPie2(){
        ...
        let myChart = this.$echarts.init(this.$refs.charts2);
        let chartData=[{
              name:"已用",
    ...
    getCpu(){
      let that=this;
      that.$myHttp.get('systemMonitor/getCpuInfo').then((res) => {
          that.cpuData=res.result;
      }).then(()=>{
          that.drawPie3();
      });
    },
    drawPie3(){
        // 基于准备好的dom，初始化echarts实例
        let myChart = this.$echarts.init(this.$refs.charts3);
        let chartData=[{
              name:"已用",
              value:parseFloat(this.cpuData.used).toFixed(2)
            },{
              name:"未用",
              value:100-parseFloat(this.cpuData.used).toFixed(2)
            }];
        // 绘制图表
        myChart.setOption({
            color: ["#10c4B5", "#005562"],
            tooltip: {
              ...
              title: {
                    text:parseFloat(this.cpuData.used).toFixed(2)+"%",
                    left: 'center',
                    textStyle: {
                      color: '#767676',
                      fontSize: 14,
...
.box>h3{color:#ffffff;height:20px;line-height: 20px;font-weight: normal;font-size:20px;margin-bottom: 20px;padding-left:20px;}
.box>h3:first-child{margin-top:20px;}
...
.container2,.container3{width:30%;height:150px;overflow:hidden;background:#ffffff;color:#767676;margin:0 20px;box-sizing: border-box;border-radius: 10px;display:flex;justify-content: space-between;flex-wrap: wrap;align-items: center;}
.container2{margin-bottom:20px;}
#charts2,#charts3{width:50%;height:150px;}
</style>
