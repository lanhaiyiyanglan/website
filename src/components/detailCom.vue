...
export default {
  ...
  methods: {
    ...
    async changeRoute(value, obj) {
      console.log('routeparams',value,obj);
     ...
     return new Promise(resolve=>{
              ...
              this.$myHttp.get('pointManage/getStakesByRouteId?routeId=' + value).then((res) => {
                    ...
                    if(temp.length>0){
                      let tempArr=[];
                      temp.map((item)=>{
                         tempArr.push({"STAKEID":item.STAKEID});
                      })
                      this.takeList =this.deteleObject(tempArr);
                      if (this.comtype == 'edit') {
                        let arr = [];
                        let arr2=[];
                        ...
                        arr.map((item)=>{
                           arr2.push({"STAKEID":item.STAKEID,"LATITUDE":item.LATITUDE,"LONGITUDE":item.LONGITUDE});
                        })
                        this.stakes = this.deteleObject(arr2);
                      }
                    }
